...
import subprocess
import os
import shutil # zip 파일 생성을 위한 모듈
import tempfile
import json
import shutil
import time
...
PY_ASD = 'run_ASD.py'   # 'run_ASD.py'
PY_STT = 'stt_aws.py'   # 'stt_whisper.py'
...
# def run_subprocess(name, target='ko'):
#     """
#     ASD, STT, SBS를 실행
#     """
#     subprocess.run(['python', PY_ASD, '--videoName', name.split('.')[0]], cwd=ASD_CWD)             # ASD 실행
#     subprocess.run(['python', PY_STT, '--videoName', name], cwd=MAIN_CWD)                          # STT 실행
#     subprocess.run(['python', PY_MER, '--videoName', name, '--language', target], cwd=MAIN_CWD)    # Subtitle Sync 실행
...
#         # 파일 확장자 확인
#         file_extension = name.split('.')[1]
#         if file_extension == 'avi':             # AVI 파일이면 MP4로 변환
#             name = convert_avi_to_mp4(name)

#         # ASD, STT, SBS 실행
#         run_subprocess(name, target)
